...
// ===== DEBUG EXPORT (download game snapshot as JSON) =====
async function exportRaidData() {
  if (!gameId) return alert("Geen gameId.");
  ...
  const [gSnap, pSnap, lSnap, aSnap] = await Promise.all([
    getDoc(gameDocRef),
    getDocs(query(playersRef)),
    getDocs(query(logRef, orderBy("createdAt", "asc"))),
    getDocs(query(actionsRef, orderBy("createdAt", "asc"))),
  ]);

  const payload = {
    exportedAt: new Date().toISOString(),
    gameId,
    game: gSnap.exists() ? { id: gSnap.id, ...gSnap.data() } : null,
    players: pSnap.docs.map(d => ({ id: d.id, ...d.data() })),
    log: lSnap.docs.map(d => ({ id: d.id, ...d.data() })),
    actions: aSnap.docs.map(d => ({ id: d.id, ...d.data() })),
  };
  ...
  a.href = url;
  a.download = `vossenjacht_${gameId}_export.json`;
  a.click();
  URL.revokeObjectURL(url);
}

// expose to button or console
window.exportRaidData = exportRaidData;

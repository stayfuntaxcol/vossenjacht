...
function buildExplanation(best, upcoming, view) {
  const bullets = [];

  if (best.decision === "BURROW" && (view.me?.burrowRemaining ?? 1) === 1) {
    bullets.unshift(
      "Let op: Je kunt BURROW maar 1 keer gebruiken per Raid. Dus alleen doen als het echt nodig is."
    );
  }
...
export function getAdvisorHint({ game, me, players, profileKey = "BEGINNER_COACH" }) {
  const profile =
  ...
  return {
    ...
    confidence,
    risk: best.riskLabel,
    bullets: buildExplanation(best, upcoming, view),
